...
c-----------------------------------------------------------------------
      DEALLOCATE(be,be_n0,ja,ve,pres,pres_n0,prese,nd,conc,
     $  work1,work2,work3,be_eq,ja_eq,ve_eq,pres_eq,prese_eq,
     $  tele_eq,tion_eq,nd_eq,diff_shape,si_nl_pres,tele,tion)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      IF (nrbl>0) THEN
        DO ibl=1,nrbl
          ...
          CALL matrix_rbl_dealloc(cmass_mat%rbl_mat(ibl))
...
c-----------------------------------------------------------------------
      IF (nbl>nrbl) THEN
        DO ibl=nrbl+1,nbl
          ...
          CALL matrix_tbl_dealloc(cmass_mat%tbl_mat(ibl))
...
c-----------------------------------------------------------------------
      DEALLOCATE(cell_rhs,cell_crhs,rhs,crhs,sln,csln,vectr,cvectr)
c-----------------------------------------------------------------------
c     deallocate preconditioning factor structures.
c-----------------------------------------------------------------------
      CALL iter_fac_dealloc(mass_fac,nrbl,solver)
      CALL iter_fac_dealloc(cmass_fac,nrbl,solver)
c-----------------------------------------------------------------------
c     terminate.
c-----------------------------------------------------------------------
      RETURN
