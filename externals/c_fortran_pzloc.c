 ...
 * for calling the SuperLU_DIST routine pzgssvx with distributed
 * matrix storage.
 */
...
#include "config.h"
#include "int_size.h"
#if defined HAVE_SUPERLU_DIST5
#include "superlu_defs.h"
#define superlu_options_t superlu_dist_options_t
#endif
#if defined HAVE_SUPERLU_DIST6
#include "superlu_defs.h"
#define superlu_options_t superlu_dist_options_t
#endif
...
int_t 
c_fortran_pzloc_(int *iopt, int_t *n, int_t *mloc, int_t *nnzloc,
                 int_t *fstrow, int *nrhs,
                 doublecomplex *values, int_t *colind, int_t *rowptr,
...
/* end of definitions for temp writes */
	...
	zCreate_CompRowLoc_Matrix_dist(&A, *n, *n, *nnzloc, *mloc, *fstrow,
	...
	pzgssvx(&options, &A, ScalePermstruct, NULL, *ldb, 0, 
	...
	*factors = (long) LUfactors;
	...
	Destroy_LU(*n, grid, LUfactors->LUstruct);
        ...
        options.SolveInitialized = YES;
        options.RefineInitialized = NO;
        zSolveFinalize(&options, LUfactors->SOLVEstruct);
	ScalePermstructFree(LUfactors->ScalePermstruct);
        SUPERLU_FREE(LUfactors);

    } else {

	fprintf(stderr, "Invalid iopt=%d passed to c_fortran_pzloc()\n",*iopt);
	exit(-1);
