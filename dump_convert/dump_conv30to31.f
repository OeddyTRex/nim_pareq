...
c-----------------------------------------------------------------------
      ...
      INTEGER(i4) :: ib,i,ibasis,im,ix,iy,ix0,iy0
...
c-----------------------------------------------------------------------
      ...
      ENDDO
c-----------------------------------------------------------------------
c     allocate and fill fields that have been added since version 3_0.
c-----------------------------------------------------------------------
      DO ib=1,nrbl
        ...
        DO ibasis=1,poly_degree**2
          ...
          DO iy=iy0,rb(ib)%my
            DO ix=ix0,rb(ib)%mx
              ...
              CALL lagr_quad_eval(rb(ib)%pres ,ix-ix0+dx,iy-iy0+dy,0_i4)
              CALL lagr_quad_eval(rb(ib)%prese,ix-ix0+dx,iy-iy0+dy,0_i4)
              DO im=1,nmodes
                CALL lagr_quad_basis_assign_loc(rb(ib)%tele,
     $             rb(ib)%prese%f(:,im)/(kboltz*rb(ib)%nd_eq%f(1)),
     $             ibasis,ix,iy,im)
                CALL lagr_quad_basis_assign_loc(rb(ib)%tion,
     $            (rb(ib)%pres%f(:,im)-rb(ib)%prese%f(:,im))/
     $             (kboltz*rb(ib)%nd_eq%f(1)),
     $            ibasis,ix,iy,im)
              ENDDO
            ENDDO
          ENDDO
...
c-----------------------------------------------------------------------
c     there are no temperatures in 3_0 dump files.
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     terminate.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
c     17.  main program, dump_conv30to31
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     declarations.
c-----------------------------------------------------------------------
      PROGRAM dump_conv30to31
      USE local
      USE dump_con_mod
      USE fields
      IMPLICIT NONE
...
c-----------------------------------------------------------------------
      WRITE(nim_wr,*) "Enter the names of the old (nimrod3_0) dump file"
      WRITE(nim_wr,*) "and the new (nimrod3_1+) dump file."
