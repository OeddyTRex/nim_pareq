c-----------------------------------------------------------------------
c     file iter_precon_comp.f
c     This set of modules holds nimrod's traditional preconditioning
c     routines for complex 2D algebraic systems over the meshed plane.
c     these routines had been the bulk of iter_cg_comp.f.  The CG solver
c     module for complex 2D systems, itself, appears separately in
c     the iter_cg_comp.f file.  See that file for documentation of
c     older changes to the preconditioners that are now here.
c     1/10/19, C. Sovinec
c-----------------------------------------------------------------------
c     code organization.
c-----------------------------------------------------------------------
c     module iter_comp_direct
c     1. fac_dir.
c     2. solve_dir.
c     module iter_comp_line
c     3. iter_line_fac
c     4. iter_line_solve
c     5. iter_ld_fac
c     6. iter_ld_solve
...
c     10. iter_ld_elim
c     11. iter_inner_pr
c     module iter_comp_fac
c     12. iter_fac_alloc_comp.
c     13. iter_fac_dealloc_comp.
c     14. iter_factor_comp.
c     15. iter_mat_com.
c     16. iter_mat_rest.
c     17. iter_pre_comp (external).
c-----------------------------------------------------------------------
c     module for the direct solve preconditioner.
c-----------------------------------------------------------------------
      MODULE iter_comp_direct
      USE local
      USE matrix_type_mod
      USE factor_type_mod
      USE pardata
...
c-----------------------------------------------------------------------
      ...
      TYPE :: it1d_ctype
        COMPLEX(r8), DIMENSION(:), ALLOCATABLE :: data
      END TYPE it1d_ctype

      INTEGER(i4) :: iopt,ierr,ist,ien
      INTEGER(i4), PARAMETER :: nrhs=1
      COMPLEX(r8), DIMENSION(:), ALLOCATABLE :: acc,atmp
      COMPLEX(r8) :: bdum
      REAL(r8) :: timefac_st,timefac_en
      INTEGER(i4), PARAMETER :: mpi_nim_iter = mpi_nim_comp

      CHARACTER(128) :: msg

      TYPE(it1d_ctype), DIMENSION(:), POINTER :: recvstr,sendstr
...
c-----------------------------------------------------------------------
      ...
      IF (precon=='lapack') THEN
        IF (.NOT.single_pr)  THEN
          CALL zpbtrf('L',spp%nrow,spp%nbw,fac%a11(1,1),
     $                spp%nbw+1_i4,ierr)
        ELSE
          CALL cpbtrf('L',spp%nrow,spp%nbw,fac%a11(1,1),
     $                spp%nbw+1_i4,ierr)
        ENDIF
        IF (ierr/=0) THEN
          WRITE(msg,'(a,i9)') 'Fac_dir comp: **zpbtrf** ierr=',ierr
          CALL nim_stop(msg)
        ENDIF
      ELSE IF (precon=='seq_slu') THEN
        CALL c_fortran_zgssv(
     $         iopt,spp%nrow,spp%nnz,nrhs,acc,spp%j_acc,
     $         spp%start_acc,bdum,spp%nrow,spp%acc_handle,ierr)
        IF (ierr==-999999) THEN
          CALL nim_stop
     $      ('Fac_dir comp: the sequential SLU library is not linked.')
        ELSE IF (ierr/=0) THEN
          WRITE(msg,'(a,i9)') 'Fac_dir comp: **zgssv** ierr=',ierr
          CALL nim_stop(msg)
        ENDIF
      ELSE IF (precon(1:5)=='slu_d') THEN
        IF (precon(1:7)=='slu_dst') THEN
          CALL c_fortran_pzloc(
     ...
     $           slugrid_handle,ierr)
        ...
        IF (ierr==-999999) THEN
          CALL nim_stop
     $      ('Fac_dir comp: the SLU_DIST library is not linked.')
        ELSE IF (ierr/=0) THEN
          WRITE(msg,'(a,i9)') 'Fac_dir comp: **pzloc fac** ierr=',ierr
          CALL nim_stop(msg)
        ENDIF
      ENDIF
...
c-----------------------------------------------------------------------
     ...
     $       spp%algcntr(node_layer),mpi_nim_comp,btmp(0),
     $       spp%algcntr(0),spp%algdsplr(0),mpi_nim_comp,
     $       comm_layer,ierr)
        bb=btmp
        DEALLOCATE(btmp)
      ENDIF
...
c-----------------------------------------------------------------------
      ...
      IF (precon=='lapack') THEN
        IF (.NOT.single_pr)  THEN
          CALL zpbtrs('L',spp%nrow,spp%nbw,1_i4,fac%a11(1,1),
     $                spp%nbw+1_i4,bb(0),spp%nrow,ierr)
        ELSE
          CALL cpbtrs('L',spp%nrow,spp%nbw,1_i4,fac%a11(1,1),
     $                spp%nbw+1_i4,bb(0),spp%nrow,ierr)
        ENDIF
        IF (ierr/=0) THEN
          WRITE (msg,'(a,i9)') 'Solve_dir comp: **zpbtrs** ierr=',ierr
          CALL nim_stop(msg)
        ENDIF
      ELSE IF (precon=='seq_slu') THEN
        CALL c_fortran_zgssv(iopt,spp%nrow,spp%nnz,nrhs,accdum,
     $                       spp%j_acc,spp%start_acc,bb,spp%nrow,
     $                       spp%acc_handle,ierr)
        IF (ierr/=0) THEN
          WRITE (msg,'(a,i9)') 'Solve_dir comp: **zgssv** ierr=',ierr
          CALL nim_stop(msg)
        ENDIF
      ELSE IF (precon(1:5)=='slu_d') THEN
        IF (precon(1:7)=='slu_dst') THEN
     ...
     $           slugrid_handle,spp%iopts,spp%dopts,ierr)
        ELSE
          CALL c_fortran_pzglob(iopt,spp%nrow,spp%nnz,nrhs,accdum,
     $                      spp%j_acc,spp%start_acc,bb,INT(spp%nrow,i4),
     $                      spp%acc_handle,slugrid_handle,ierr)
        ENDIF
        IF (ierr/=0) THEN
          WRITE (msg,'(a,i9)') 'Solve_dir comp: **pzloc** ierr=',ierr
          CALL nim_stop(msg)
        ENDIF
      ENDIF
...
c-----------------------------------------------------------------------
     ...
     $                  spp%fstrow
          ...
          ENDIF
          CALL edge_load_2D_carr(zee(ibl),nqty,pdm1,seam(ibl))
        ENDDO
        CALL edge_network(nqty,1_i4,pdm1,.false.)
        DO ibl=1,nbl
          CALL edge_unload_2D_carr(zee(ibl),nqty,pdm1,seam(ibl))
        ENDDO
c-----------------------------------------------------------------------
...
c     close module.
c-----------------------------------------------------------------------
      END MODULE iter_comp_direct


c-----------------------------------------------------------------------
c     module for line-Jacobi type preconditioners.
c-----------------------------------------------------------------------
      MODULE iter_comp_line
      USE local
      USE math_tran
      USE edge_type_mod
      USE matrix_type_mod
...
c-----------------------------------------------------------------------
      ...
      COMPLEX(r8), DIMENSION(:,:,:,-offlim:,:), INTENT(INOUT) :: mat

      INTEGER(i4) :: iq,jq,ix,nend,ioff
      INTEGER(i4), PARAMETER :: noff=5
      REAL(r8) :: offinc
      REAL(r8), DIMENSION(SIZE(mat,1)) :: off
      COMPLEX(r8), DIMENSION(SIZE(mat,1),1:1,nqty,nqty) :: lu,bb
      COMPLEX(r8), DIMENSION(SIZE(mat,1),1:1,nqty) :: xx
      COMPLEX(r8), DIMENSION(SIZE(mat,1),SIZE(mat,2),SIZE(mat,3),
...
c-----------------------------------------------------------------------
      ...
      off_loop: DO
        ...
        singular=.false.
        lu(:,1,:,:)=mat(:,:,:,0,1)
        CALL math_solve_herm(nqty,lu,xx,bb(:,:,:,1),'factor',singular)
        IF (singular) CALL nim_stop
     $    ('Iter_line_fac unable to factor first point-block.')
        DO iq=1,nqty
          CALL math_solve_herm(nqty,lu,xx,bb(:,:,:,iq),'solve',singular)
          mat(:,:,iq,0,1)=xx(:,1,:)
        ENDDO
        lu(:,1,:,:)=mat(:,:,:,1,1)
        DO jq=1,nqty
     ...
     $           -SUM(mat(:,iq,:,-1,ix)*mat(:,:,jq, 1,ix-1),2)
          ...
          DO iq=1,nqty
            CALL math_solve_herm(nqty,lu,xx,bb(:,:,:,iq),
     $                           'solve',singular)
            mat(:,:,iq,0,ix)=xx(:,1,:)
          ENDDO
          lu(:,1,:,:)=mat(:,:,:,1,ix)
...
c-----------------------------------------------------------------------
      ...
      INTEGER(i4), INTENT(IN) :: nqty,nmx,offlim
      COMPLEX(r8), DIMENSION(:,:,:,-offlim:,:), INTENT(IN) :: mat
      COMPLEX(r8), DIMENSION(:,:,:), INTENT(INOUT) :: zz

      INTEGER(i4) :: ix,iq,jq,nend
      COMPLEX(r8), DIMENSION(SIZE(mat,1),nqty) :: tmp
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $               jst,iof,jof,xad,yad,xst,yst,npdm,ibase,elen
      COMPLEX(r8), DIMENSION(:,:,:,:), POINTER, CONTIGUOUS :: lc,cl
      COMPLEX(r8), DIMENSION(:,:,:,:,:), POINTER, CONTIGUOUS ::
     $             adix,adiy
      COMPLEX(r8), DIMENSION(:,:,:,:,:,:), POINTER, CONTIGUOUS :: lmat
      LOGICAL :: sing
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $                         poly_d2,use_int)
      ...
      TYPE(bl_comp_fac_type), INTENT(IN) :: bl_fac
      TYPE(cvector_2D_type), INTENT(INOUT) :: zeebl,resbl
     ...
     $               xad,yad,npdm,iym1,jbar2,perpb_max,jx,jy,ib0,ib1,iv
      COMPLEX(r8), DIMENSION(:,:,:), ALLOCATABLE :: zz,zh
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
c     subprogram 7. iter_solve_herm.
...
c     decomposition is returned in lu.  if the instruction is
...
c-----------------------------------------------------------------------
      ...
      IF (instruction/='solve') THEN
        IF (REAL(lu(1,1),r8)<=0) THEN
          singular=.true.
          RETURN
        ENDIF
        lu(1,1)=SQRT(REAL(lu(1,1),r8))
        DO i=2,n1
...
c-----------------------------------------------------------------------
     ...
     $      -SUM(lu(jcol,1:jcol-1)*CONJG(lu(jcol,1:jcol-1)))
          ...
          DO i=jcol+1,n1
            lu(i,jcol)=(lu(i,jcol)
     $          -SUM(lu(i,1:jcol-1)*CONJG(lu(jcol,1:jcol-1))))
     $              /lu(jcol,jcol)
          ENDDO
        ENDDO
      ENDIF
...
c-----------------------------------------------------------------------
      DO k=1,n2
        y(1)=bb(1,k)/lu(1,1)
        DO i=2,n1
          y(i)=(bb(i,k)-SUM(lu(i,1:i-1)*y(1:i-1)))/lu(i,i)
        ENDDO
        xx(n1,k)=y(n1)/lu(n1,n1)
        DO i=n1-1,1,-1
          xx(i,k)=(y(i)-SUM(CONJG(lu(i+1:n1,i))*xx(i+1:n1,k)))/lu(i,i)
        ENDDO
      ENDDO
c-----------------------------------------------------------------------
c     terminate.
c-----------------------------------------------------------------------
      RETURN
      END SUBROUTINE iter_solve_herm
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $               itype,jtype,iof,ibst,iben
      COMPLEX(r8), DIMENSION(:,:,:), POINTER, CONTIGUOUS :: btmp
      COMPLEX(r8), DIMENSION(:,:,:,:), POINTER, CONTIGUOUS :: lc,cl
      COMPLEX(r8), DIMENSION(:,:,:,:,:), POINTER, CONTIGUOUS :: adi
      COMPLEX(r8), DIMENSION(:,:,:,:,:,:), POINTER, CONTIGUOUS :: lmat
      REAL(r8) :: timeline_st,timeline_en
      TYPE(cvector_2D_type), DIMENSION(nrb) :: blck_tmp,line_tmp
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      IF (poly_deg>1) THEN
        ...
        DO ibl=1,ntotb
          DO iv=1,seam(ibl)%nvert
            seam(ibl)%vertex(iv)%seam_cin(1:nq2)=0
            seam(ibl)%segment(iv)%seam_cin(1:(poly_deg-1)*nq2)=0
          ENDDO
        ENDDO
        DO ibl=1,nrb
          myb=fac%bl_fac(ibl)%my
          ...
          DO iy=1,myb
            ...
            DO jq=1,nq
              DO iq=1,nq
                jy=jy+1
                seam(ibl)%vertex(iv)%seam_cin(jy)=
     ...
     $                              poly_deg-1_i4,ibst,iben)
              DO ibase=ibst,iben,seam(ibl)%segment(iv)%load_dir
                ...
                DO jq=1,nq
                  DO iq=1,nq
                    jy=jy+1
                    seam(ibl)%segment(iv)%seam_cin(jy)=
     ...
     $            SUM(cl(:,iq,1,0)*lc(jq,:,1,0))
        ...
        ENDDO
        IF (mat%eliminated) THEN
          CALL edge_network(nq2,1_i4,0_i4,.false.)
          perpb_max=poly_deg
        ELSE
          CALL edge_network(nq2,1_i4,poly_deg-1_i4,.false.)
...
c-----------------------------------------------------------------------
     ...
     $        seam(ibl)%vertex(iv)%seam_cout(1:nq2)
          ENDDO
          IF (.NOT.mat%eliminated) THEN
     ...
     $            btmp(nq2+1:2*nq2,mxb,iy+yad)-
     $              seam(ibl)%segment(iv)%seam_cout(jq+1:jq+nq2)
              ENDDO
              jq=jq+nq2
            ENDDO
          ENDIF
...
c-----------------------------------------------------------------------
      ...
      DO ibl=1,nrb
        ...
        DO iy=1,myb
          iv=seam(ibl)%nvert-iy
          seam(ibl)%vertex(iv)%seam_cin(1:nq2)=btmp(2*nq2+1:3*nq2,0,iy)
        ENDDO
        IF (seam(ibl)%segment(1)%ptr(1)<=0.OR.
     $      seam(ibl)%segment(1)%ptr(1)>nrb_allpr) THEN
          iv=seam(ibl)%nvert
          seam(ibl)%vertex(iv)%seam_cin(1:nq2)=btmp(2*nq2+1:3*nq2,0,0)
        ENDIF
     ...
     $                        load_dir,poly_deg-1_i4,ibst,iben)
        ...
        DO ibase=ibst,iben,seam(ibl)%segment(seam(ibl)%nvert)%load_dir
          DO iy=ibase*myb+1,(ibase+1)*myb
            ...
            seam(ibl)%segment(iv)%seam_cin(jq+1:jq+nq2)=
...
c-----------------------------------------------------------------------
      ...
      CALL parallel_line_comm_bl2line_comp(blck_tmp,line_tmp,nrb,ntotb,
     $                                     3_i4*nq2,linex)
...
c-----------------------------------------------------------------------
        ...
        DO ibl=1,nrb
          ...
          DO ix=1,mxb
            ...
            DO jq=1,nq
              DO iq=1,nq
                jx=jx+1
                seam(ibl)%vertex(ix)%seam_cin(jx)=
     ...
     $                              poly_deg-1_i4,ibst,iben)
              DO ibase=ibst,iben,seam(ibl)%segment(ix)%load_dir
                ...
                DO jq=1,nq
                  DO iq=1,nq
                    jx=jx+1
                    seam(ibl)%segment(ix)%seam_cin(jx)=
     ...
     $        seam(ibl)%segment(iv)%ptr(1)>nrb_allpr) THEN
            ...
            DO jq=1,nq
              DO iq=1,nq
                jx=jx+1
                seam(ibl)%vertex(iv)%seam_cin(jx)=
...
c-----------------------------------------------------------------------
      ...
      DO ibl=1,nrb
        ...
        mxb=fac%bl_fac(ibl)%mx
        DO ix=1,mxb
          seam(ibl)%vertex(ix)%seam_cin(1:nq2)=btmp(2*nq2+1:3*nq2,ix,0)
        ENDDO
        iv=seam(ibl)%nvert
        IF (seam(ibl)%segment(iv)%ptr(1)<=0.OR.
     $      seam(ibl)%segment(iv)%ptr(1)>nrb_allpr) THEN
...
c-----------------------------------------------------------------------
     ...
     $                            poly_d2,ntotb,use_int)
      ...
      TYPE(complex_factor_type), INTENT(IN) :: fac
      INTEGER(i4), INTENT(IN) :: nq,nrb,poly_deg,poly_d2,ntotb
      TYPE(cvector_2D_type), DIMENSION(ntotb), INTENT(INOUT) :: res,zee,
     $                       adr
      LOGICAL, INTENT(IN) :: use_int
     ...
     $               itype,jtype,ib0,ib1,jx,jy,jbarr,npdm,ibst,iben
      TYPE(cvector_2D_type), DIMENSION(nrb) :: zz,rx,rxh,ry,ryv
      REAL(r8) :: timeline_st,timeline_en
c-----------------------------------------------------------------------
c     if cell-interior data has been eliminated from the matrix equation
c     before calling the iterative solver, it is not used here.
...
c-----------------------------------------------------------------------
     ...
     $        seam(ibl)%segment(1)%ptr(1)>nrb_allpr) xad=0
          DO iv=1,seam(ibl)%nvert
            ...
            seam(ibl)%vertex(iv)%seam_cin(1:2*nq)=0
            IF (use_int)
     $        seam(ibl)%segment(iv)%seam_cin(1:2*nq*(poly_deg-1))=0
            IF (ix==0.AND.iy>=xad) THEN
               DO jq=1,nq
                  seam(ibl)%vertex(iv)%seam_cin(jq)=
     $              seam(ibl)%vertex(iv)%seam_cin(jq)+
            ...
            IF (use_int.AND..NOT.seam(ibl)%segment(iv)%h_side) THEN
              ...
              IF (ix==0) THEN
                ...
                DO jbase=ibst,iben,seam(ibl)%segment(iv)%load_dir
                  yad=myb*jbase
                  DO jq=1,nq
                    seam(ibl)%segment(iv)%seam_cin(jof+jq)=
     $                seam(ibl)%segment(iv)%seam_cin(jof+jq)+
     ...
     $        seam(ibl)%segment(nv)%ptr(1)>nrb_allpr) yad=0
          DO iv=1,nv
            ...
            IF (iy==0.AND.ix>=yad) THEN
              DO jq=1,nq
                seam(ibl)%vertex(iv)%seam_cin(jq+nq)=
     $            seam(ibl)%vertex(iv)%seam_cin(jq+nq)+
...
c-----------------------------------------------------------------------
     ...
     $        seam(ibl)%segment(iv)%seam_cin(1:2*nq*(poly_deg-1))=0
        ...
        ENDDO
        IF (use_int) THEN
          CALL edge_network(2_i4*nq,1_i4,poly_deg-1_i4,.false.)
        ELSE
          CALL edge_network(2_i4*nq,1_i4,0_i4,.false.)
        ENDIF
        DO ibl=1,nrb
          mxb=fac%bl_fac(ibl)%mx
          myb=fac%bl_fac(ibl)%my
     ...
     $          seam(ibl)%vertex(iv)%seam_cout(1:nq)
            ENDIF
            IF (iy==0.OR.iy==myb) THEN
              ry(ibl)%arr(:,ix,iy)=ry(ibl)%arr(:,ix,iy)-
     $          seam(ibl)%vertex(iv)%seam_cout(nq+1:2*nq)
            ENDIF
            IF (use_int) THEN
              ix=seam(ibl)%segment(iv)%intxys(1)
              iy=seam(ibl)%segment(iv)%intxys(2)
     ...
     $              seam(ibl)%segment(iv)%seam_cout(jof+1:jof+nq)
                  jof=jof+2*nq
                ENDDO
              ELSE
                jof=0
...
c-----------------------------------------------------------------------
      CALL timer(timeline_st)
      IF (poly_deg==1) THEN
        CALL parallel_line_comm_bl2line_comp(res,zz,nrb,ntotb,nq,linex)
      ELSE
        CALL parallel_line_comm_bl2line_comp(rx,zz,nrb,ntotb,nq,linex)
      ENDIF
      CALL timer(timeline_en)
      time_line = time_line + timeline_en-timeline_st
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      CALL timer(timeline_st)
      IF (poly_deg==1) THEN
        CALL parallel_line_comm_line2bl_comp(zee,zz,nrb,ntotb,nq,linex)
      ELSE
        CALL parallel_line_comm_line2bl_comp(rx,zz,nrb,ntotb,nq,linex)
      ENDIF
      CALL timer(timeline_en)
      time_line = time_line + timeline_en-timeline_st
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      CALL timer(timeline_st)
      IF (poly_deg==1) THEN
        CALL parallel_line_comm_bl2line_comp(res,zz,nrb,ntotb,nq,liney)
      ELSE
        CALL parallel_line_comm_bl2line_comp(ry,zz,nrb,ntotb,nq,liney)
      ENDIF
      CALL timer(timeline_en)
      time_line = time_line + timeline_en-timeline_st
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      CALL timer(timeline_st)
      IF (poly_deg==1) THEN
        CALL parallel_line_comm_line2bl_comp(adr,zz,nrb,ntotb,nq,liney)
      ELSE
        CALL parallel_line_comm_line2bl_comp(ry,zz,nrb,ntotb,nq,liney)
      ENDIF
      CALL timer(timeline_en)
      time_line = time_line + timeline_en-timeline_st
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $               line_elmax,perpb_max,itype,jtype,nqo,iqo,jqo
      COMPLEX(r8), DIMENSION(:,:), ALLOCATABLE :: xx
      COMPLEX(r8), DIMENSION(:,:,:), ALLOCATABLE :: cc,id
      LOGICAL :: sing
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $                arr(iof+1:iof+npdm,0,0,iof+1:iof+npdm,ix,iy)
              ENDDO
            ENDDO
            CALL math_solve_q1_herm(npdm,SIZE(cc,3),cc,xx,id(:,:,1),
     $                              'factor',sing)
            IF (sing) CALL nim_stop
     $        ('Iter_ld_elim: high-order diag does not factor.')
            DO iq=1,npdm
              CALL math_solve_q1_herm(npdm,SIZE(cc,3),cc,xx,id(:,:,iq),
     $                                'solve',sing)
...
c-----------------------------------------------------------------------
      ...
      COMPLEX(r8), DIMENSION(*), INTENT(IN) :: arr1,arr2
...
c-----------------------------------------------------------------------
      SUBROUTINE iter_fac_alloc_comp(mat_str,fac_str,nqty,precon,
     $                               int_elim)
      USE pardata
      USE iter_dir_fac, ONLY: iter_dirfac_alloc

      TYPE(complex_matrix_type), INTENT(IN) :: mat_str
      TYPE(complex_factor_type), INTENT(OUT) :: fac_str
...
c-----------------------------------------------------------------------
     ...
     $  CALL nim_stop
     $    ('iter_fac_alloc: preconditioner choice is not recognized.')
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $      fac_str%spp%iopts,fac_str%spp%dopts,ierr)
        ELSE IF (precon=='slu_dist') THEN
          CALL c_fortran_pzglob(iopt,fac_str%spp%nrow,fac_str%spp%nnz,
...
c-----------------------------------------------------------------------
      RETURN
      END SUBROUTINE iter_fac_dealloc_comp
c-----------------------------------------------------------------------
c     subprogram 14. iter_factor_comp.
...
c     reused.
c-----------------------------------------------------------------------
      SUBROUTINE iter_factor_comp(mat_str,fac_str,nqty,precon,
     $                            off_diag_fac)
      USE pardata
      USE time
     ...
     $               poly_deg,poly_d2,nbtype,nrb,ntotb,ndiag
      INTEGER(i4), DIMENSION(:), ALLOCATABLE :: shif
      INTEGER(i4), DIMENSION(:,:), ALLOCATABLE :: offt,offt2
      COMPLEX(r8), DIMENSION(:,:), ALLOCATABLE :: lu,iden,tmp
      COMPLEX(r8), DIMENSION(:,:,:), ALLOCATABLE :: iden2,tmp2
      COMPLEX(r8), DIMENSION(:,:,:,:), ALLOCATABLE :: lu2
...
c-----------------------------------------------------------------------
     ...
     $                      poly_deg,poly_d2,ntotb)
      ...
      ENDIF
c-----------------------------------------------------------------------
c     copy diagonal elements for diagonal preconditioning.
c-----------------------------------------------------------------------
      DO ibl=1,nrb
...
c     (zee and res are used as temporary storage).  the first index
c     is the row index for optimization; hence, the conjg(transp())
c     of the inverse is stored.
c-----------------------------------------------------------------------
      DO ibl=1,ntotb
        IF (precon=='diagonal'.OR.ibl>nrb) THEN
          itype_dst=1
          ...
          IF (mat_str%hermitian) THEN
            ...
            DO iy=iy0,my
              DO ix=ix0,mx
                ...
                CALL iter_solve_herm(nqd,nqd,lu,tmp,
     $                               iden,'both',singular)
                IF (singular) CALL nim_stop
     $            ('Iter_factor_comp unable to factor point-block.')
                fac_str%bl_fac(ibl)%pmat(itype)%arr(:,:,ix,iy)=
     $            TRANSPOSE(CONJG(tmp))
              ENDDO
            ENDDO
            DEALLOCATE(iden,lu,tmp)
          ELSE !  use newer math_tran solves for non-Hermitian mats.
            ALLOCATE(lu2(nqd,nqd,ix0:mx,iy0:my))
            ALLOCATE(iden2(nqd,ix0:mx,iy0:my))
            ALLOCATE(tmp2(nqd,ix0:mx,iy0:my))
            ndiag=(mx+1-ix0)*(my+1-iy0)
            lu2=fac_str%bl_fac(ibl)%pmat(itype)%arr
            CALL math_solve_q1_cnxn(nqd,ndiag,lu2,tmp2,iden2,'factor',
     $                              singular)
            IF (singular) CALL nim_stop
     $        ('Iter_factor_comp unable to factor non-Herm diag.')

            DO iq=1,nqd
              iden2=0._r8
              iden2(iq,:,:)=1._r8
              CALL math_solve_q1_cnxn(nqd,ndiag,lu2,tmp2,iden2,
     $                                'solve',singular)
              fac_str%bl_fac(ibl)%pmat(itype)%arr(iq,:,:,:)=tmp2
            ENDDO
            DEALLOCATE(lu2,iden2,tmp2)
          ENDIF
        ENDDO
      ENDDO
...
c-----------------------------------------------------------------------
     ...
     $        dg32(joff+1:joff+nqty,:,2,idg)
      ...
      ENDDO
c-----------------------------------------------------------------------
c     load seam_cin from perimeter of matrix arrays, rblocks first.
c-----------------------------------------------------------------------
      rblock_in: DO ibl=1,nrb
        ...
        DO iv=1,seam(ibl)%nvert
          ...
          DO iqty=1,nqty
            DO jqty=1,nqty
              n=n+1
              seam(ibl)%vertex(iv)%seam_cin(n)=rmat(jqty,0,0,iqty,ix,iy)
            ENDDO
          ENDDO
          seam(ibl)%vertex(iv)%seam_csave(1:n)=
     $      seam(ibl)%vertex(iv)%seam_cin(1:n)
        ENDDO
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $                            ibst,iben)
            DO ibase=1,nb
              ...
              DO iqty=1,nqty
                DO jqty=1,nqty
                  nm=nm+1
                  seam(ibl)%segment(iv)%seam_csave(nm)=
     $              rmat(iqo+jqty,0,0,iqo+iqty,ix,iy)
            ...
            nm=1
            DO ibase=ibst,iben,seam(ibl)%segment(iv)%load_dir
              seam(ibl)%segment(iv)%seam_cin(nq2*(ibase-1)+1:nq2*ibase)=
     $          seam(ibl)%segment(iv)%seam_csave(nm:nm+nq2-1)
              nm=nm+nq2
            ENDDO
          ENDDO
        ENDIF
          ...
          IF (seam(ibl)%segment(iv)%h_side) THEN
            ...
            DO ibase=poly_deg+1,2,-1
              ...
              DO jbase=-1,-(ibase-1),-1
                ...
                seam(ibl)%segment(iv)%seam_mat_cin(1:nqty,1:nqty,ioff)
...
c-----------------------------------------------------------------------
     ...
     $          mat%tbl_mat(ibl)%lmat(ix)%element(jqty,iqty,0)
        ...
        DO iv=1,seam(ibl)%nvert
          ...
          ixp=seam(ibl)%segment(iv)%intxyp(1)
          iyp=seam(ibl)%segment(iv)%intxyp(2)
          seam(ibl)%segment(iv)%seam_mat_cin(1:nqty,1:nqty,1)=
     $      mat%tbl_mat(ibl)%lmat(ix)%element(1:nqty,1:nqty,iy)
          seam(ibl)%segment(iv)%seam_mat_cin(1:nqty,1:nqty,2)=
     $      mat%tbl_mat(ibl)%lmat(ixp)%element(1:nqty,1:nqty,iyp)
        ENDDO
      ENDDO tblock_in
c-----------------------------------------------------------------------
c     copy seam_mat_cin information into the seam_save arrays for
...
c-----------------------------------------------------------------------
      DO ibl=1,ntotb
        DO iv=1,seam(ibl)%nvert
          seam(ibl)%segment(iv)%seam_mat_csave
     $      (1:nqty,1:nqty,1:poly_d2+poly_deg)=
     $        seam(ibl)%segment(iv)%seam_mat_cin
     $          (1:nqty,1:nqty,1:poly_d2+poly_deg)
        ENDDO
      ENDDO
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      CALL edge_network(n,1_i4,poly_deg-1_i4,.false.)
      CALL edge_seg_network(nqty,0_i4,poly_d2+poly_deg,.true.)
c-----------------------------------------------------------------------
c     unload seam_cout into matrix arrays.
c-----------------------------------------------------------------------
     ...
     $             seam(ibl)%vertex(iv)%seam_cout(n)
            ENDDO
          ENDDO
        ENDDO
c-----------------------------------------------------------------------
            ...
            DO ibase=ibst,iben,seam(ibl)%segment(iv)%load_dir
              ...
              DO iqty=1,nqty
                DO jqty=1,nqty
                  nm=nm+1
                  rmat(iqo+jqty,0,0,iqo+iqty,ix,iy)=
     $              seam(ibl)%segment(iv)%seam_cout(nm)
                ENDDO
              ENDDO
            ENDDO
          ENDDO
          ...
          IF (seam(ibl)%segment(iv)%h_side) THEN
            ...
            DO ibase=poly_deg+1,2,-1
              ...
              DO jbase=-1,-(ibase-1),-1
                ...
                joff=mat%rbl_mat(ibl)%ix0(jtyp)-1
                rmat(jqo+1:jqo+nqty,joff,0,iqo+1:iqo+nqty,ixs,iys)=
     $           seam(ibl)%segment(iv)%seam_mat_cout(1:nqty,1:nqty,ioff)
                ioff=ioff+2
              ENDDO
            ENDDO
            ioff=1+ixs-ixp
     ...
     $             seam(ibl)%vertex(iv)%seam_cout(n)
        ...
        IF (precon/='diagonal') THEN
          DO iv=1,seam(ibl)%nvert
            ...
            iyp=seam(ibl)%segment(iv)%intxyp(2)
            mat%tbl_mat(ibl)%lmat(ix)%element(1:nqty,1:nqty,iy)=
     $         seam(ibl)%segment(iv)%seam_mat_cout(1:nqty,1:nqty,1)
            mat%tbl_mat(ibl)%lmat(ixp)%element(1:nqty,1:nqty,iyp)=
     $         seam(ibl)%segment(iv)%seam_mat_cout(1:nqty,1:nqty,2)
          ENDDO
        ENDIF
      ENDDO tblock_out
c-----------------------------------------------------------------------
...
c     restore the border elements of the matrix from seam_csave.
c-----------------------------------------------------------------------
     ...
     $               idg,ndg,nqpm1,nqp1p,nqpd,my
      COMPLEX(r8), DIMENSION(:,:,:,:,:,:), POINTER :: rmat
c-----------------------------------------------------------------------
c     unload seam_csave into matrix arrays.
c-----------------------------------------------------------------------
      rblock_out: DO ibl=1,nrb
        rmat=>mat%rbl_mat(ibl)%mat(1,1)%arr
        DO iv=1,seam(ibl)%nvert
          ...
          DO iqty=1,nqty
            DO jqty=1,nqty
              n=n+1
              rmat(jqty,0,0,iqty,ix,iy)=
     $          seam(ibl)%vertex(iv)%seam_csave(n)
            ENDDO
          ENDDO
        ENDDO
c-----------------------------------------------------------------------
     ...
     $          seam(ibl)%vertex(iv)%seam_csave(n)
        ...
        IF (precon/='diagonal') THEN
          DO iv=1,seam(ibl)%nvert
            ...
            iyp=seam(ibl)%segment(iv)%intxyp(2)
            mat%tbl_mat(ibl)%lmat(ix)%element(1:nqty,1:nqty,iy)=
     $        seam(ibl)%segment(iv)%seam_mat_csave(1:nqty,1:nqty,1)
            mat%tbl_mat(ibl)%lmat(ixp)%element(1:nqty,1:nqty,iyp)=
     $        seam(ibl)%segment(iv)%seam_mat_csave(1:nqty,1:nqty,2)
          ENDDO
        ENDIF
      ENDDO tblock_out
c-----------------------------------------------------------------------
...
c     close module.
c-----------------------------------------------------------------------
      END MODULE iter_comp_fac


c-----------------------------------------------------------------------
c     subprogram 17. iter_pre_comp
...
c-----------------------------------------------------------------------
      SUBROUTINE iter_pre_comp(mat_str,fac_str,resp,zeep,adrp,nqty,nrb,
     $                         ntotb,precon)
      USE local
      USE matrix_type_mod
      USE factor_type_mod
      ...
      CHARACTER(*), INTENT(IN) :: precon
      TYPE(cvector_2D_type), DIMENSION(ntotb), INTENT(INOUT) :: resp,
     $                       zeep,adrp
     ...
     $               nqd,mxb,myb,poly_deg,poly_d2
      ...
      COMPLEX(r8), DIMENSION(:,:,:,:), POINTER, CONTIGUOUS :: pmatptr
      COMPLEX(r8), DIMENSION(:,:,:), ALLOCATABLE :: zeeptr,resptr
      LOGICAL, EXTERNAL :: direct_check
c-----------------------------------------------------------------------
...
c     data at cell interiors.  duplication with iter_pre_comp allows
c     direct calls to iter_pre_comp.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      DO ibl=rsstart,ntotb
        DO iv=1,seam(ibl)%nvert
          ix=seam(ibl)%vertex(iv)%intxy(1)
          iy=seam(ibl)%vertex(iv)%intxy(2)
          seam(ibl)%vertex(iv)%seam_csave(1:nqty)=
     $      resp(ibl)%arr(:,ix,iy)
     ...
     $      seam(ibl)%vertex(iv)%ave_factor_pre
          IF (poly_deg>1) THEN
            ...
            IF (seam(ibl)%segment(iv)%h_side) THEN
              DO ibase=1,poly_deg-1
                seam(ibl)%segment(iv)%seam_csave(iq:iq+nqty-1)=
...
c-----------------------------------------------------------------------
     ...
     $                 ntotb,precon)
      ELSE IF (precon(1:7)=='bl_diag') THEN
        DO ibl=1,nrb
          CALL iter_ld_solve(fac_str%bl_fac(ibl),zeep(ibl),
     $      resp(ibl),nqty,precon,poly_deg,poly_d2,use_int)
        ENDDO
      ELSE IF (precon=='gl_diaga') THEN
...
c-----------------------------------------------------------------------
      IF (.NOT.direct_check(precon)) THEN
        DO ibl=1,rsstart-1
          CALL edge_load_2D_carr(zeep(ibl),nqty,poly_deg-1_i4,seam(ibl))
          DO iv=1,seam(ibl)%nvert
     ...
     $          seam(ibl)%segment(iv)%ave_factor_pre
        ...
        DO ibl=1,ntotb
          CALL edge_unload_2D_carr(zeep(ibl),nqty,poly_deg-1_i4,
     $                             seam(ibl))
        ENDDO
      ENDIF
c-----------------------------------------------------------------------
