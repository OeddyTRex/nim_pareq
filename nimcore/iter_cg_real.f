c-----------------------------------------------------------------------
c     file iter_cg_real.f  (formerly iter_cg_f90.f)
...
c     preconditioner for real matrices and vectors.
c
c     4/22/97  D. Barnes-C. Sovinec-S. Plimpton
c     dbarnes@lanl.gov sovinec@lanl.gov
c
c     also updated 8/6/97 to set initial guess for domain decomp
c     with the preconditioner and residual.
c
c     9/8/97 new features include communication of off-diagonal elements
c     of matrices along block boundaries and block-incomplete
c     factorization schemes.  C. Sovinec
c
c     9/9/97 array order in rb%lmat and vector index move in tb%lmat
c     reflect optimization changes in the rest of nimrod.  also, the
c     rblock_mvcmpy calls pass entire arrays instead of segments, which
c     makes a surprising difference.  C. Sovinec
c
c     9/26/97 the restart is now similar to Dan's original version,
c     however it has yet to demonstrate improved convergence.  thus,
c     nrst is set to a large number.  C. Sovinec
c
c     10/10/97 diagonal preconditioning now stores the inverse of the
c     diagonal sub-matrix for nqty up to 6.  C. Sovinec
c
c     3/11/98 rhs is no longer used as a working array.  C. Sovinec
c
c     4/10/98 the preconditioner factor allocation and computation
c     (iter_fac_alloc and iter_factor, respectively) have been
c     separated from iter_init, so they may be saved elsewhere.
c
c     4/15/98 the dummy index for tblock arrays has been changed to 0
c     for consistency with rblocks, and new rhs & sln structures
c     have been incorporated.  this reduces the number of separate
c     rblock and tblock loops.
c
c     5/29/98 averaging of preconditioning steps at block boundaries is
c     now done in a symmetric fashion.  this was preventing convergence
c     with any preconditioner besides diagonal on multi-block problems.
c     iter_line_fac and math_solve_sym have also been modified to
c     search for optimum off_diag factors for each grid line.
c
c     6/10/98 iter_ilu_solve, iter_ld_solve and blk_pc have been
c     modified to be called within a block loop with vectors passed.
c     this facilitates calls from iter_cg_2, where real vector
c     structures are not defined.  also, the adi routines have been
c     removed, since they have never worked.
c
c     6/15/98 iter_solve_sym now reports if a factorization breaks
c     down, in which case, nim_stop is called.  something similar
c     has already been done for math_solve_sym.
c
c     11/4/98 the rblock matrix order has been changed so that grid
c     offsets are first to optimize caching on the t3e.
c
c     12/16/98 this module and some routines have been renamed, as
c     an option to use a complex version is now available.  the
c     iter_cg module is the switchyard for selecting real or complex
c     routines.  also, the ilu factorization now decouples the
c     degenerate point unless the degenerate block is also periodic.
c
...
c     or passed into iter_pre_real, so that it may be called from
...
c     8/30/06, C. Sovinec
c
c     a no-preconditioning option has been added for tests.
...
c     the iter_solve_sym routine is retained with the 'ilu' module.
c     8/26/13, C. Sovinec
c
...
c     and it is now named iter_cg_r2d_solve.
c     1/13/19, C. Sovinec
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     code organization.
c-----------------------------------------------------------------------
c     module iter_cg_real
c     1. iter_cg_r2d_solve.
c     2. iter_init.
c     3. iter_dealloc.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      CONTAINS
c-----------------------------------------------------------------------
c     subprogram 1. iter_cg_r2d_solve.
c     controls the conjugate gradient iterations.
c-----------------------------------------------------------------------
      SUBROUTINE iter_cg_r2d_solve(mat_str,fac_str,sln,rhs,nqty,tol,
     $                             maxit,precon,err,its,seed)
      ...
      TYPE(vector_type), DIMENSION(:), POINTER :: res,zee,dir,adr,slnl
      INTEGER(i4) :: ibl,it,irst
      INTEGER(i4) :: ix,iy,iv,mv,iq
      INTEGER(i4), PARAMETER :: nrst=10000
      REAL(r8) :: alpha,beta,er0,erl,ztr,ztro,ercheck
...
c    $        "Iter_cg_f90: restarting at it ",it," err=",err
          ELSE
...
c    $          "Iter_cg_f90: correcting residual it ",
...
c-----------------------------------------------------------------------
        CALL iter_pre_real(mat_str,fac_str,res,zee,adr,nqty,nrb,ntotb,
     $                     precon)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $                  poly_deg,use_int)
        ...
        IF (nprocs_layer > 1) THEN
          CALL mpi_allreduce(ztr,tmp,1,mpi_nim_real,mpi_sum,
     $         comm_layer,ierror)
          ztr=tmp
        ENDIF
        beta=ztr/ztro
...
c-----------------------------------------------------------------------
     ...
     $                  poly_deg,use_int)
        ENDDO
        IF (nprocs_layer > 1) THEN
          CALL mpi_allreduce(alpha,tmp,1,mpi_nim_real,mpi_sum,
     $         comm_layer,ierror)
          alpha = tmp
        ENDIF
        alpha=ztr/alpha
...
c     CALL mpi_barrier(comm_layer,ierror)
      ...
      RETURN
      END SUBROUTINE iter_cg_r2d_solve
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $                     res,zee,dir,adr,slnl)
      ...
      TYPE(vector_type), DIMENSION(:), POINTER :: rhs,sln
      TYPE(vector_type), DIMENSION(:), POINTER :: res,zee,dir,adr,slnl
...
c-----------------------------------------------------------------------
      IF (.NOT.direct_check(precon)) THEN
        DO ibl=1,ntotb
          ...
          ENDIF
        ENDDO
        CALL iter_pre_real(mat,fac,res,zee,adr,nqty,nrb,ntotb,precon)
      ENDIF
c-----------------------------------------------------------------------
