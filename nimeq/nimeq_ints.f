...
c     entry and R*alpha(jv)*alpha(iv) for the second.
c-----------------------------------------------------------------------
      DO iv=1,nv
        DO jv=1,nv
          int(1,1,:,jv,iv)=SUM((dalpdr(:,:,jv)*dalpdr(:,:,iv)+
...
c     components of ja_eq.  the 2nd component is lambda=psi/R**2,
...
c-----------------------------------------------------------------------
     ...
     $                      laj,lajr,lajz,1_i4)
c-----------------------------------------------------------------------
c     grad(alpha)*grad(lambda)*R^2 and alpha*mu0*J_phi*R:
c-----------------------------------------------------------------------
      bigr2=bigr(:,:)**2
      DO iv=1,nv
...
c-----------------------------------------------------------------------
     ...
     $                        tb%ng,blam,blamr,blamz,1_i4)
      ...
      ELSE
        ...
        bdir(2,:,:)= lamr(1,:,:)
      ENDIF
      bdir(1,:,:)=blen_check*bdir(1,:,:)/flux_n0
      bdir(2,:,:)=blen_check*bdir(2,:,:)/flux_n0
c-----------------------------------------------------------------------
