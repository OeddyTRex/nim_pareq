...
c-----------------------------------------------------------------------
      ...
      INTEGER(i4), PARAMETER :: neqcoil_max=5000
...
c     the field-line based topology check can impede convergence, where
c	small changes in the closed-region positioning kick the relative
c	error back up by orders of magnitude after it has become small.
c	this can be countered by averaging results from tracing the
c	current iterate with the traces from the previous iterate.  the
c	parameters log_rerr_fla0 and log_rerr_fla1 control the averaging
c	in the following way: when the log (base 10) of the relative
c	error is larger than log_rerr_fla0, only results from the
c	current iterate are used.  when log(rel. error) is between
c	log_rerr_fla0 and log_rerr_fla1, averaging between the old and
c	new lengths is used with increasingly old results used as
c	the log(rel. error) decreases.  below log_rerr_fla1, field-line
c	lengths are not updated.  the default does not apply this
c	averaging, but initial recommendations are shown below.

      REAL(r8) :: log_rerr_fla0=-20._r8 !  try -2
      REAL(r8) :: log_rerr_fla1=-21._r8 !  try -4 or -5
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $  curfil_file,free_symmj,ndeq_notx,rbph_notx,log_rerr_fla0,
     $  log_rerr_fla1,ode_pkg
c-----------------------------------------------------------------------
c     open input file.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $     mpi_comm_world,ierror)
      CALL mpi_bcast(log_rerr_fla0,1,mpi_nim_real,0,
     $     mpi_comm_world,ierror)
      CALL mpi_bcast(log_rerr_fla1,1,mpi_nim_real,0,
     $     mpi_comm_world,ierror)
