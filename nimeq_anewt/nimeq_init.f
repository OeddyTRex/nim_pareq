...
c-----------------------------------------------------------------------
     ...
     $                       poly_degree,'presn0',(/'presn0'/))
        CALL lagr_quad_alloc(rb(ibl)%nd_n0,mxb,myb,1_i4,
     $                       poly_degree,'nd_n0 ',(/'nd_n0 '/))
...
c-----------------------------------------------------------------------
     ...
     $                       poly_degree,'rw3',(/'rwork3 '/))
        CALL lagr_quad_alloc(rb(ibl)%rwork4,mxb,myb,1_i4,
     $                       poly_degree,'rw4',(/'rwork4 '/))
     ...
     $                       poly_degree,'be_n0',(/'be_n0 '/))
        ...
        CALL rblock_qp_alloc(rb(ibl)%qrwork4,rb(ibl),1_i4)
        rb(ibl)%work1=0
        rb(ibl)%work2=0
        rb(ibl)%pflux=0
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
c       the following are used for newton iteration.
c-----------------------------------------------------------------------
        CALL vector_type_alloc(diperr(ibl),poly_degree,mxb,myb,2_i4)
      ENDDO rblocks
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $                        (/'tioneq'/))
        CALL tri_linear_alloc(tb(ibl)%pres_n0,mv,1_i4,'presn0',
     $                        (/'presn0'/))
        CALL tri_linear_alloc(tb(ibl)%nd_n0,mv,1_i4,'nd_n0 ',
     $                        (/'nd_n0 '/))
...
c-----------------------------------------------------------------------
     ...
     $                        name='rw3',title=(/'rwork3 '/))
        CALL tri_linear_alloc(tb(ibl)%rwork4,mv,1_i4,
     $                        name='rw4',title=(/'rwork4 '/))
     ...
     $                        name='be_n0',title=(/'be_n0 '/))
        ...
        CALL tblock_qp_alloc(tb(ibl)%qrwork4,tb(ibl),1_i4)
        tb(ibl)%work1=0
        tb(ibl)%work2=0
        tb(ibl)%pflux=0
c-----------------------------------------------------------------------
...
c       the following are used for newton iteration.
c-----------------------------------------------------------------------
        CALL vector_type_alloc(diperr(ibl),1_i4,mv,0_i4,1_i4)
      ENDDO tblocks
c-----------------------------------------------------------------------
c     terminate.
c-----------------------------------------------------------------------
...
c----------------------------------------------------------------------
      ...
      END SELECT
c-TMP applied to J component, too?
...
c-----------------------------------------------------------------------
     ...
     $  pres_n0(nbl),nd_n0(nbl),prese(nbl),nd(nbl),conc(nbl),
...
c-----------------------------------------------------------------------
      DO ibl=1,nrbl
        ...
        CALL vector_ptassign_laq2(rwork4(ibl),rb(ibl)%rwork4)
      ...
      DO ibl=nrbl+1,nbl
        ...
        CALL vector_ptassign_tl2(rwork4(ibl),tb(ibl)%rwork4)
