c-----------------------------------------------------------------------
c     module 1. read_millasc.
c     reads ASCII data from Bob Miller's TOQ equilibrium code.
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE read_millasc(gt)
      USE local
      USE inverse
      USE input
      USE global
     ...
     &                                       mach_in
      REAL(r8), DIMENSION(:,:), ALLOCATABLE :: rg_in,zg_in
c-----------------------------------------------------------------------
c     the reals and integers are assumed to be 32 bit.
c-----------------------------------------------------------------------
c-TMP
c     CALL open_bin(eq1_unit,TRIM(filename),"UNKNOWN","REWIND",32_i4)
      OPEN(UNIT=eq1_unit,FILE=TRIM(filename),STATUS="UNKNOWN")
